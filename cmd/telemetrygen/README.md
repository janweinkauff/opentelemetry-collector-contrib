...
| Flag | Level | Applied to |
| ---- | ----- | ---------- |
| `--otlp-attributes` | Resource attributes | The OTLP resource, once per run. Describes the entity producing the telemetry, and is shared by every span, metric data point and log record. |
| `--telemetry-attributes` | Signal attributes | Each individual generated span, metric data point and log record. Describes the single telemetry item. |
