...
func (c *Config) CommonFlags(fs *pflag.FlagSet) {
	...
	fs.Var(&c.TelemetryAttributes, "telemetry-attributes", "Custom signal attributes, describing the individual telemetry item. They are set on every generated span, metric data point and log record rather than on the resource, e.g. --telemetry-attributes http.response.status_code=200. "+
